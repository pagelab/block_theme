  ...
  <div class="container mx-auto px-4 relative z-10"> 
    <div class="flex flex-col md:flex-row items-center justify-between"> 
      <div class="mb-8 md:mb-0 md:w-1/2"> 
        <h2 class="text-3xl md:text-4xl font-bold text-inverse mb-4"><?php echo PG_Blocks_v3::getAttribute( $args, 'heading' ) ?></h2> 
        <p class="text-gray-200 text-lg mb-6"><?php echo PG_Blocks_v3::getAttribute( $args, 'description' ) ?></p> 
        <div class="flex flex-wrap gap-4"> 
          <a href="<?php echo (!empty($_GET['context']) && $_GET['context'] === 'edit') ? 'javascript:void()' : PG_Blocks_v3::getLinkUrl( $args, 'primary_button_link' ) ?>" class="bg-inverse-subtle duration-300 font-semibold px-6 py-3 rounded-md text-inverse transition hover:bg-primary-700"><?php echo PG_Blocks_v3::getAttribute( $args, 'primary_button_text' ) ?></a> 
          <a href="<?php echo (!empty($_GET['context']) && $_GET['context'] === 'edit') ? 'javascript:void()' : PG_Blocks_v3::getLinkUrl( $args, 'secondary_button_link' ) ?>" class="button-inverse border border-white text-inverse hover:bg-white hover:text-gray-800 font-semibold py-3 px-6 rounded-md transition duration-300"><?php echo PG_Blocks_v3::getAttribute( $args, 'secondary_button_text' ) ?></a> 
        </div> 
      </div> 
      <div class="md:w-2/5"> 
        <div class="bg-base p-6 rounded-lg shadow-lg"> 
          <h3 class="text-xl font-bold text-subtle mb-4"><?php echo PG_Blocks_v3::getAttribute( $args, 'sidebar_title' ) ?></h3> 
          <ul class="space-y-3"> 
            <li class="flex items-center"> 
              <span class="bg-inverse-subtle mr-3 p-1 rounded-full text-inverse"> 
